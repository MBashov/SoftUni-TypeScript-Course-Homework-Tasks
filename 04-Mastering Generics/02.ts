...
class CountedSet<T> implements CountableSet<T> {
    ...
    add(item: T): void {
        const currenCount = this.items.get(item);
        if (currenCount) {
            this.items.set(item, currenCount + 1);
        } else {
    ...
    remove(item: T): void {
        ...
        if (currenCount !== undefined && currenCount > 0) { 
            this.items.set(item, currenCount - 1);
        }
    };
    ...
    contains(item: T): boolean {
        const currenCount = this.items.get(item);
        return currenCount !== undefined && currenCount > 0;
    };
